...
def getCost(a_tuple, b_tuple, target_tuple):
    ...
    if np.linalg.det(M) != 0:
        x = linalg.solve(M,t)
        if all(i >= 0 and abs(round(i) - i) < epsilon  for i in list(x)):
            print(x)
            return 3*x[0] + x[1]
    return 0
...
for line in f:
    ...
    if "Button B" in line:
        ...
        b_tuple = (b_x,b_y)
    if "Prize" in line:
        target_x = int(line.split(':')[1].split(',')[0].strip().split('=')[1]) + 10000000000000
        target_y = int(line.split(':')[1].split(',')[1].strip().split('=')[1]) + 10000000000000
