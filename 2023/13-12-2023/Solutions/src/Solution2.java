...
public class Solution2 {
    ...
    public Solution2(String filepath,int rowFactor,int colFactor) throws IOException {
    ...
    private boolean isMirror(Integer[] rowIndexes, ArrayList<String> pattern) {
        ...
        while (lower >= 0 && higher < pattern.size()) {
            String lowerLine = pattern.get(lower);
            String higherLine = pattern.get(higher);
            for (int i = 0; i < lowerLine.length(); i++) {
                if (lowerLine.charAt(i) != higherLine.charAt(i)) {
                    smudgesFound++;
                    if (smudgesFound > 1) {
                        return false;
                    }
